...
import { cartMongoose } from "../dao/cart/cart.mongoose.js";
...
describe('Testing Cart Routes', () => {
    ...
    it('Debe permitir agregar un nuevo carrito', async function (done) {
    ...
    it('Debe permitir agregar un producto al carrito', async function () {
        ...
        expect(res.status).to.equal(200);
        const cart = await cartMongoose.getCartById(cartId);
        expect(cart.products).to.be.an('array').that.is.not.empty;
    });

    it('Debe permitir eliminar un producto del carrito', async function (done) {
    ...
    it('Debe permitir eliminar un carrito por su ID', async function (done) {
